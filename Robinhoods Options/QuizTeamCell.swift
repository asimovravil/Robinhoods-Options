...
protocol QuizTeamCellDelegate: AnyObject {
    func didChangeText(newText: String, in cell: QuizTeamCell)
}

class QuizTeamCell: UITableViewCell {

    let teamPlaceHolder = UITextField()
    weak var delegate: QuizTeamCellDelegate?
    
    static let id = String(describing: QuizTeamCell.self)
